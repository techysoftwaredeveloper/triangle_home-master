...
class _HosterDetailScreenState extends State<HosterDetailScreen> {
  ...
  Future<void> _handleUpdate(String status) async {
    ...
    try {
      ...
      if (status == 'approved') {
        ...
        await FirebaseFirestore.instance.collection('hoster_requests').doc(uid).set({
          'status': 'approved',
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance.collection('users').doc(widget.request['uid']).snapshots(),
      builder: (context, snapshot) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            title: const Text('Partner Verification', style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Outfit')),
            centerTitle: true,
            elevation: 0,
            backgroundColor: Colors.white,
              ...
              SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildDocCard('Aadhaar Card', verif['govIdFrontUrl'], verif['govIdBackUrl'], 'govId', verif['govIdVerified'] == true),
                    _buildDocCard('PAN Card', verif['panFrontUrl'], null, 'pan', verif['panVerified'] == true),
                    const SizedBox(height: 32),
                    _buildSectionTitle('Business Documents'),
                    _buildDocCard('Business Proof', verif['businessProofFrontUrl'], null, 'business', verif['businessProofVerified'] == true),
                    _buildDocCard('Property Proof', verif['propertyProofFrontUrl'], null, 'property', verif['propertyProofVerified'] == true),
                    const SizedBox(height: 32),
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: 'Add a note or specify what info is missing...',
                        filled: true,
    ...
    return Row(
      children: [
        CircleAvatar(
          ...
          child: info['profileImage'] == null ? const Icon(Icons.person, size: 40, color: AppTheme.successColor) : null,
        ),
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(info['name'] ?? 'Unknown', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, fontFamily: 'Outfit')),
              Text(info['email'] ?? 'No email', style: const TextStyle(color: AppTheme.textLightColor)),
              Text(info['phone'] ?? 'No phone', style: const TextStyle(color: AppTheme.textLightColor)),
            ],
          ),
        ),
      ],
  ...
  Widget _buildDocCard(String title, String? frontUrl, String? backUrl, String field, bool isVerified) {
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (frontUrl != null) ...[
            ...
            const Padding(padding: EdgeInsets.only(top: 12), child: Text('No documents uploaded', style: TextStyle(color: Colors.grey, fontSize: 12))),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          ClipRRect(
            ...
            child: CachedNetworkImage(imageUrl: url, height: 80, width: double.infinity, fit: BoxFit.cover),
          ),
    ...
    return Row(
      children: [
        TextButton(
          ...
          child: const Text('Verify', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold, fontSize: 12)),
        ),
        TextButton(
  ...
  Future<void> _verifyField(String field, bool verified) async {
    ...
    if (field == 'govId') {
      ...
      update['verification.businessProofStatus'] = verified ? 'verified' : 'rejected';
      update['verification.businessProofRejectReason'] = reason;
    } else if (field == 'property') {
      update['verification.propertyProofVerified'] = verified;
      update['verification.propertyProofStatus'] = verified ? 'verified' : 'rejected';
          ...
          children: [
            Expanded(
              child: ElevatedButton(
                ...
                child: const Text('Request Info', style: TextStyle(fontWeight: FontWeight.bold)),
              ),
            ),
            ...
            Expanded(
              child: ElevatedButton(
                ...
                child: const Text('Approve Account', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ),
          ],
        ),
