...
class _ModerationTabState extends State<ModerationTab> with SingleTickerProviderStateMixin {
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final filteredLogs = allLogs.where((l) {
          final matchesSearch = (l['id']?.toString().toLowerCase().contains(_searchQuery) ?? false) ||
                                (l['action']?.toString().toLowerCase().contains(_searchQuery) ?? false) ||
                                (l['targetId']?.toString().toLowerCase().contains(_searchQuery) ?? false) ||
                                (l['reason']?.toString().toLowerCase().contains(_searchQuery) ?? false);

          final type = l['targetType']?.toString().toLowerCase() ?? '';

          switch (_tabController.index) {
            case 1: return matchesSearch && type == 'property';
            case 2: return matchesSearch && type == 'users';
            case 3: return matchesSearch && type == 'review';
            case 4: return matchesSearch && type == 'message';
        ...
        return Row(
          children: [
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    TabHeader(
                      title: 'Moderation',
                      subtitle: 'Review and take action on content, users and listings',
                    ...
                    if (snapshot.connectionState == ConnectionState.waiting && allLogs.isEmpty)
            ...
            if (!widget.isNarrow && _selectedItem != null)
              _buildDetailPanel(),
          ],
        );
      }
  ...
  void _handleExport() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Exporting audit logs to CSV...'), backgroundColor: Color(0xFF2563EB)),
    );
  }
      ...
      child: Row(
        children: [
          ...
          SummaryCard(
            ...
            sub: logs.isEmpty ? '0%' : '${((pendingCount/logs.length)*100).toStringAsFixed(1)}% of total',
          ),
          const SizedBox(width: 16),
          SummaryCard(
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: widget.isNarrow ? 'Search...' : 'Search by id, property, user or reason...',
  ...
  Widget _buildTableHeader() {
    return Padding(
      ...
      child: Row(
        children: [
          const SizedBox(width: 24, child: Icon(Icons.check_box_outline_blank_rounded, size: 18, color: Color(0xFFCBD5E1))),
          const SizedBox(width: 16),
  ...
  Widget _buildModerationList(List<Map<String, dynamic>> logs) {
    return Column(
      children: logs.map((l) => _ModerationRow(
        id: l['id']?.toString().substring(0, 8).toUpperCase() ?? 'MOD-NEW',
        ...
        statusColor: (l['status'] == 'Pending') ? Colors.orange : Colors.green,
  ...
  String _formatAction(dynamic action) {
    ...
    return a.split(' ').map((s) => s.isNotEmpty ? s[0].toUpperCase() + s.substring(1) : '').join(' ');
  }

  String _getPriority(dynamic action) {
    final a = action?.toString().toLowerCase() ?? '';
    if (a.contains('delete') || a.contains('block') || a.contains('ban')) return 'High';
  ...
  void _handleAction(String id, String action) {
     ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Moderation action "$action" on $id coming soon'), backgroundColor: const Color(0xFF2563EB)),
    );
  }
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                const Text('Audit Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                IconButton(icon: const Icon(Icons.close, size: 20), onPressed: () => setState(() => _selectedItem = null)),
              ],
            ),
            const SizedBox(height: 24),
            ...
            ClipRRect(
              ...
              child: Stack(
                children: [
                  Container(height: 180, width: double.infinity, color: const Color(0xFFF1F5F9), child: const Icon(Icons.shield_outlined, color: Colors.grey, size: 40)),
                  Positioned(top: 12, right: 12, child: StatusBadge(text: l['status'] ?? 'Resolved', color: (l['status'] == 'Pending') ? Colors.orange : Colors.green)),
                ],
              ),
            ),
            const SizedBox(height: 20),

            Text(action, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
            const SizedBox(height: 4),
            Row(
              children: [
                Text(_formatType(l['targetType']), style: const TextStyle(fontSize: 12, color: Color(0xFF64748B))),
                const SizedBox(width: 8),
                const Text('•', style: TextStyle(color: Colors.grey)),
                const SizedBox(width: 8),
                Text('Target ID: ${l['targetId']?.toString().toUpperCase()}', style: const TextStyle(fontSize: 12, color: Color(0xFF475569), fontWeight: FontWeight.bold)),
              ],
            ),
            ...
            _detailRow('Timestamp', _formatDateTime(l['timestamp']).replaceAll('\n', ' ')),

            const SizedBox(height: 16),
            const Text('Reason / Note', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF94A3B8), letterSpacing: 1)),
            const SizedBox(height: 4),
            Text(l['reason'] ?? 'Routine automated audit log entry.', style: const TextStyle(fontSize: 12, color: Color(0xFF64748B), height: 1.5)),

            const SizedBox(height: 16),
            const Text('Priority Level', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF94A3B8), letterSpacing: 1)),
            const SizedBox(height: 8),
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: priorityColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.flag_rounded, size: 12, color: priorityColor), const SizedBox(width: 4), Text(_getPriority(l['action']), style: TextStyle(color: priorityColor, fontSize: 10, fontWeight: FontWeight.bold))])),

            const Divider(height: 48),
            _buildDetailSectionTitle('System Context'),
            const Text('No additional security context provided for this entry.', style: TextStyle(fontSize: 11, color: Colors.grey)),

            const SizedBox(height: 40),
            _actionButton('Acknowledge Log', const Color(0xFFF0FDF4), const Color(0xFF16A34A), Icons.check_circle_outline),
            const SizedBox(height: 12),
            _actionButton('Reverse Action', const Color(0xFFFEF2F2), const Color(0xFFDC2626), Icons.history_rounded),
            const SizedBox(height: 12),
            _actionButton('Escalate to Dev', const Color(0xFFEFF6FF), const Color(0xFF2563EB), Icons.code_rounded),
          ],
        ),
      ),
    );
  ...
  Widget _actionButton(String label, Color bg, Color text, IconData icon) {
    return Container(
      ...
      decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(10), border: Border.all(color: text.withValues(alpha: 0.1))),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
  ...
  Widget _buildPaginationFooter(int count) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(widget.isNarrow ? '1-$count of $count' : 'Showing 1 to $count of $count items', style: const TextStyle(fontSize: 11, color: Color(0xFF64748B))),
        Row(
          children: [
...
class _ModerationRow extends StatelessWidget {
  ...
  const _ModerationRow({required this.id, required this.type, required this.subject, this.subjectId, required this.reportedBy, required this.reason, required this.priority, required this.priorityColor, required this.status, required this.statusColor, required this.date, required this.isSelected, required this.onTap, required this.isNarrow, required this.onAction});
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: isNarrow
          ? Column(children: [
              Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(id, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)), Text(subject, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF1E293B)), maxLines: 1, overflow: TextOverflow.ellipsis)])),
          ...
          : Row(children: [
              ...
              const SizedBox(width: 16),
              // 1. ID
              Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(id, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))), Text(type, style: const TextStyle(fontSize: 10, color: Colors.grey))])),
              // 2. Type
              Expanded(flex: 2, child: Row(children: [Container(width: 32, height: 32, decoration: BoxDecoration(color: const Color(0xFFF1F5F9), borderRadius: BorderRadius.circular(6)), child: Icon(type == 'Listing' ? Icons.home_work_outlined : Icons.person_outline, size: 16, color: Colors.grey))])),
              // 3. Subject
              Expanded(flex: 3, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(subject, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF1E293B)), maxLines: 1, overflow: TextOverflow.ellipsis), if(subjectId != null) Text(subjectId!, style: const TextStyle(fontSize: 10, color: Colors.grey))])),
              // 4. Reported By
              Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(reportedBy, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)), const Text('Admin', style: TextStyle(fontSize: 9, color: Colors.grey))])),
              // 5. Reason
              Expanded(flex: 3, child: Text(reason, style: const TextStyle(fontSize: 11, color: Color(0xFF475569)), maxLines: 2, overflow: TextOverflow.ellipsis)),
              // 6. Priority
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  ...
                  const PopupMenuItem(value: 'reverse', child: Text('Reverse Action')),
