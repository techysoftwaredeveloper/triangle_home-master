...
class _ReportTabState extends State<ReportTab>
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final filteredReports =
            allReports.where((r) {
              final matchesSearch =
                  (r['id']?.toString().toLowerCase().contains(_searchQuery) ??
                      false) ||
                  (r['title']?.toString().toLowerCase().contains(
                        _searchQuery,
                      ) ??
                      false) ||
                  (r['reporterName']?.toString().toLowerCase().contains(
                        _searchQuery,
                      ) ??
                      false) ||
                  (r['targetName']?.toString().toLowerCase().contains(
                        _searchQuery,
                      ) ??
                      false);
              ...
              switch (_tabController.index) {
                ...
                case 2:
                  return matchesSearch && status == 'resolved';
                case 3:
  ...
  void _handleExport() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Exporting report audit log...'),
  ...
  Widget _buildSummaryCards(List<Map<String, dynamic>> reports) {
    ...
    final resolved =
        reports
            .where(
              (r) => (r['status'] ?? '').toString().toLowerCase() == 'resolved',
            )
            .length;
    final rejected =
        reports
            .where(
              (r) => (r['status'] ?? '').toString().toLowerCase() == 'rejected',
            )
            .length;
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText:
                          widget.isNarrow
                              ? 'Search...'
                              : 'Search by report ID, title, user or property...',
  ...
  Widget _tableLabel(String text) => Text(
    text,
    style: const TextStyle(
      fontSize: 10,
      fontWeight: FontWeight.bold,
  ...
  void _handleReportAction(String id, String action) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Report updated to $action'),
    ...
    return Row(
      ...
      children: [
        Text(
          widget.isNarrow
              ? '1-$count of $count'
              : 'Showing 1 to $count of $count reports',
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Center(
                child: Icon(icon, color: const Color(0xFFCBD5E1), size: 40),
              ),
            ),
          ],
        ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _summaryLeg(
            'Pending',
            '$pending (${reports.isNotEmpty ? ((pending / reports.length) * 100).toStringAsFixed(1) : 0}%)',
            Colors.orange,
          ),
          _summaryLeg(
            'Resolved',
            '$resolved (${reports.isNotEmpty ? ((resolved / reports.length) * 100).toStringAsFixed(1) : 0}%)',
            Colors.green,
          ),
          _summaryLeg(
            'Rejected',
            '$rejected (${reports.isNotEmpty ? ((rejected / reports.length) * 100).toStringAsFixed(1) : 0}%)',
            Colors.red,
          ),
          _summaryLeg(
            'In Progress',
            '$inProgress (${reports.isNotEmpty ? ((inProgress / reports.length) * 100).toStringAsFixed(1) : 0}%)',
            Colors.blue,
          ),
        ],
      ),
  ...
  Widget _summaryLeg(String l, String v, Color c) => Padding(
  ...
  Widget _typeItem(IconData i, String l, String v, String p, Color c) =>
      Padding(
  ...
  Widget _buildTopReportedProperties(List<Map<String, dynamic>> reports) {
    ...
    for (var r in reports) {
      if (r['category']?.toString().toLowerCase().contains('property') ==
              true ||
          r['category']?.toString().toLowerCase().contains('listing') == true) {
      ...
      child: Column(
        ...
        children: [
          ...
          if (top3.isEmpty)
            ...
            ...top3.map(
              (e) => _propertyReportItem(e.key, '${e.value} Reports'),
            ),
          const SizedBox(height: 16),
          const Center(
            child: Text(
  ...
  Widget _propertyReportItem(String name, String count) => Padding(
    padding: const EdgeInsets.only(bottom: 16),
    child: Row(
      children: [
        Container(
