...
class _SuggestionsTabState extends State<SuggestionsTab> with SingleTickerProviderStateMixin {
  ...
  Widget _buildHeader() {
    return Row(
      ...
      children: [
        ...
        Row(
          children: [
            _buildActionBtn('Export', Icons.file_download_outlined, isOutline: true, onPressed: _handleExport),
  ...
  Widget _buildActionBtn(String label, IconData icon, {bool isOutline = false, bool hasDropdown = false, VoidCallback? onPressed}) {
    return InkWell(
      onTap: onPressed,
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: isOutline ? null : [BoxShadow(color: const Color(0xFF2563EB).withValues(alpha: 0.2), blurRadius: 8, offset: const Offset(0, 4))],
        ),
        child: Row(
          children: [
  ...
  void _handleExport() {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Exporting suggestions to CSV...')));
  }
  ...
  Widget _buildSummaryRow(List<Map<String, dynamic>> suggestions) {
    ...
    String getPct(int count) => suggestions.isEmpty ? '0%' : '${((count / suggestions.length) * 100).toStringAsFixed(1)}%';
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          _StatCard(count: suggestions.length.toString(), label: 'Total Suggestions', icon: Icons.lightbulb_outline_rounded, iconColor: const Color(0xFF3B82F6), bgColor: const Color(0xFFEFF6FF), trend: '16.4%', isTrendUp: true),
          const SizedBox(width: 16),
          _StatCard(count: underReview.toString(), label: 'Under Review', icon: Icons.remove_red_eye_outlined, iconColor: const Color(0xFF10B981), bgColor: const Color(0xFFECFDF5), sub: '${getPct(underReview)} of total'),
          const SizedBox(width: 16),
          _StatCard(count: contacted.toString(), label: 'Contacted', icon: Icons.phone_in_talk_outlined, iconColor: const Color(0xFFF59E0B), bgColor: const Color(0xFFFFF7ED), sub: '${getPct(contacted)} of total'),
          const SizedBox(width: 16),
          _StatCard(count: shortlisted.toString(), label: 'Shortlisted', icon: Icons.star_outline_rounded, iconColor: const Color(0xFF8B5CF6), bgColor: const Color(0xFFF5F3FF), sub: '${getPct(shortlisted)} of total'),
          const SizedBox(width: 16),
          _StatCard(count: converted.toString(), label: 'Converted', icon: Icons.home_work_outlined, iconColor: const Color(0xFF0EA5E9), bgColor: const Color(0xFFF0F9FF), sub: '${getPct(converted)} of total'),
          const SizedBox(width: 16),
          _StatCard(count: rejected.toString(), label: 'Rejected', icon: Icons.cancel_outlined, iconColor: const Color(0xFFEF4444), bgColor: const Color(0xFFFEF2F2), sub: '${getPct(rejected)} of total'),
        ],
      ),
    );
  }
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(child: TextField(controller: _searchController, decoration: const InputDecoration(hintText: 'Search by property name, location or user...', border: InputBorder.none, hintStyle: TextStyle(fontSize: 14, color: Color(0xFF94A3B8))))),
  ...
  Widget _buildSuggestionsList(List<Map<String, dynamic>> items) {
    return Column(
      children: items.map<Widget>((item) => _SuggestionTableCard(
        item: item, 
  ...
  Widget _buildEmptyState() {
    return Container(height: 200, width: 1100, alignment: Alignment.center, child: const Text('No matching suggestions found', style: TextStyle(color: Color(0xFF64748B))));
  }
          ...
          return Column(
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Flexible(child: Text(isCompact ? '1-10 of $count' : 'Showing 1 to ${count < 10 ? count : 10} of $count suggestions', style: const TextStyle(fontSize: 13, color: Color(0xFF64748B)), overflow: TextOverflow.ellipsis)),
  ...
  Widget _buildSideSheet() {
    ...
    return Container(
      width: 400, decoration: const BoxDecoration(color: Colors.white, border: Border(left: BorderSide(color: Color(0xFFE2E8F0)))),
      child: Column(
        children: [
          Padding(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('Suggestion Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF0F172A))),
                IconButton(icon: const Icon(Icons.close, size: 20, color: Color(0xFF64748B)), onPressed: () => setState(() => _selectedSuggestion = null)),
              ],
            ),
          ),
          Expanded(
                ...
                children: [
                  ClipRRect(
                    ...
                    child: Stack(
                      children: [
                        Image.network(
                          s['property_image'] ?? '', height: 200, width: double.infinity, fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) => Container(height: 200, color: const Color(0xFFF1F5F9), child: const Icon(Icons.home_work_outlined, size: 40, color: Colors.grey)),
                        ),
                        Positioned(top: 16, right: 16, child: _StatusPill(status: s['status']?.toString() ?? 'pending')),
                      ],
                    ),
                  ),
                  const SizedBox(height: 20),
                  Text(s['business_name'] ?? 'Untitled Property', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF0F172A))),
                  const SizedBox(height: 4),
                  Row(
                    children: [
                      Text(s['category'] ?? 'PG / Hostel', style: const TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                      const SizedBox(width: 8),
                      Container(width: 4, height: 4, decoration: const BoxDecoration(color: Color(0xFFCBD5E1), shape: BoxShape.circle)),
                      const SizedBox(width: 8),
                      Text('${s['rooms_count'] ?? 30} Rooms', style: const TextStyle(fontSize: 13, color: Color(0xFF6366F1), fontWeight: FontWeight.bold)),
                    ],
                  ),
                  
                  const Divider(height: 48),
                  _sheetSection('Location'),
                  Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.location_on, size: 18, color: Color(0xFF6366F1)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(s['business_address'] ?? 'Location N/A', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF0F172A))), Text(s['sub_location'] ?? '', style: const TextStyle(fontSize: 12, color: Color(0xFF64748B)))]))]),

                  const Divider(height: 48),
                  _sheetSection('Suggested By'),
                  Row(
                    children: [
                      CircleAvatar(radius: 20, backgroundColor: const Color(0xFFF3E8FF), child: Text((s['suggester_name'] ?? 'U')[0], style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF8B5CF6)))),
                      const SizedBox(width: 12),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(s['suggester_name'] ?? 'Unknown User', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF0F172A))), Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: const Color(0xFFEEF2FF), borderRadius: BorderRadius.circular(4)), child: const Text('STUDENT', style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: Color(0xFF6366F1))))])),
                      const Text('View Profile', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF2563EB))),
                    ],
                  ),
                  const SizedBox(height: 12),
                  Text(s['suggester_phone'] ?? 'No phone', style: const TextStyle(fontSize: 13, color: Color(0xFF0F172A))),
                  Text(s['suggester_email'] ?? 'No email', style: const TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  ...
                  Text(s['description'] ?? 'No description provided.', style: const TextStyle(fontSize: 13, color: Color(0xFF475569), height: 1.5)),

                  const SizedBox(height: 48),
                  _sheetSection('Actions'),
                  _sheetActionBtn('Mark as Contacted', Icons.phone_in_talk_outlined, const Color(0xFFF5F3FF), const Color(0xFF6366F1), () => _handleSuggestionAction(s['id'], 'contacted')),
                  const SizedBox(height: 12),
                  _sheetActionBtn('Mark as Shortlisted', Icons.star_outline_rounded, const Color(0xFFEEF2FF), const Color(0xFF4F46E5), () => _handleSuggestionAction(s['id'], 'shortlisted')),
                  const SizedBox(height: 12),
                  _sheetActionBtn('Mark as Converted', Icons.check_circle_outline, const Color(0xFFECFDF5), const Color(0xFF10B981), () => _handleSuggestionAction(s['id'], 'converted')),
                  const SizedBox(height: 12),
                  _sheetActionBtn('Reject Suggestion', Icons.cancel_outlined, const Color(0xFFFEF2F2), const Color(0xFFEF4444), () => _handleSuggestionAction(s['id'], 'rejected')),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Lead status updated to $action successfully'), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget _sheetRow(String label, String value) => Padding(padding: const EdgeInsets.only(bottom: 12), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label, style: const TextStyle(fontSize: 13, color: Color(0xFF64748B))), Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF0F172A)))]));
...
class _FilterBtn extends StatelessWidget {
  ...
  const _FilterBtn({required this.label, this.hasDropdown = false}) : icon = null;
  @override
  Widget build(BuildContext context) {
    return Container(
...
class _SuggestionTableCard extends StatelessWidget {
  ...
  const _SuggestionTableCard({required this.item, required this.isSelected, required this.onTap});
  @override
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Expanded(flex: 3, child: Row(children: [ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.network(item['property_image'] ?? '', width: 44, height: 44, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(width: 44, height: 44, color: const Color(0xFFF1F5F9), child: const Icon(Icons.home_work_outlined, size: 20, color: Colors.grey)))), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item['business_name'] ?? 'Untitled Suggestion', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF0F172A)), maxLines: 1, overflow: TextOverflow.ellipsis), Text(item['category'] ?? 'PG / Hostel', style: const TextStyle(fontSize: 11, color: Color(0xFF64748B))), Text('${item['rooms_count'] ?? 30} Rooms', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF94A3B8)))]))])),
            Expanded(flex: 3, child: Row(children: [const Icon(Icons.location_on_outlined, size: 14, color: Color(0xFF94A3B8)), const SizedBox(width: 8), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item['business_address'] ?? 'Location N/A', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF0F172A)), maxLines: 1, overflow: TextOverflow.ellipsis), Text(item['sub_location'] ?? '', style: const TextStyle(fontSize: 11, color: Color(0xFF64748B)), maxLines: 1, overflow: TextOverflow.ellipsis)]))])),
            Expanded(flex: 2, child: Row(children: [CircleAvatar(radius: 14, backgroundColor: const Color(0xFFF1F5F9), child: Text((item['suggester_name'] ?? 'U')[0], style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey))), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item['suggester_name'] ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Color(0xFF0F172A)), maxLines: 1, overflow: TextOverflow.ellipsis), const Text('STUDENT', style: TextStyle(fontSize: 9, color: Color(0xFF6366F1), fontWeight: FontWeight.bold)), Text(item['suggester_phone'] ?? '', style: const TextStyle(fontSize: 10, color: Color(0xFF94A3B8)))]))])),
            Expanded(flex: 2, child: Center(child: _StatusPill(status: status))),
            Expanded(flex: 2, child: Text(item['createdAt'] is Timestamp ? DateFormat('dd MMM yyyy\nHH:mm a').format((item['createdAt'] as Timestamp).toDate()) : 'N/A', textAlign: TextAlign.right, style: const TextStyle(fontSize: 11, color: Color(0xFF64748B)))),
...
class _StatusPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (s == 'converted') { label = 'Converted'; color = const Color(0xFF10B981); bg = const Color(0xFFECFDF5); }
    else if (s == 'contacted') { label = 'Contacted'; color = const Color(0xFFF59E0B); bg = const Color(0xFFFFF7ED); }
    else if (s == 'shortlisted') { label = 'Shortlisted'; color = const Color(0xFF6366F1); bg = const Color(0xFFEEF2FF); }
    else if (s == 'rejected') { label = 'Rejected'; color = const Color(0xFFEF4444); bg = const Color(0xFFFEF2F2); }
    else { label = 'Under Review'; color = const Color(0xFF10B981); bg = const Color(0xFFECFDF5); }
