...
class EscrowService {
  ...
  Future<void> createEscrow({
    ...
    await _firestore.collection('escrow').doc(bookingId).set(record.toFirestore());
  ...
  /// Checks if a booking is ready for payout (48h after check-in, no disputes)
  ...
  Stream<EscrowRecord?> getEscrowStream(String bookingId) {
    return _firestore.collection('escrow').doc(bookingId).snapshots().map((doc) {
