...
class TimelineAggregatorService {
  ...
  Stream<List<TimelineEvent>> getUnifiedTimeline(String bookingId) {
    ...
    return Rx.combineLatest5(
      ...
      disputeEvents,
      adminActions,
      (QuerySnapshot bSnap, QuerySnapshot fSnap, QuerySnapshot pSnap, QuerySnapshot dSnap, QuerySnapshot aSnap) {
        ...
        for (var doc in pSnap.docs) {
          ...
          events.add(TimelineEvent(
            ...
            severity: data['event']?.toString().contains('FAILED') == true 
  ...
  TimelineSeverity _getBookingSeverity(String event) {
    if (event.contains('rejected') || event.contains('cancelled') || event.contains('expired')) {
