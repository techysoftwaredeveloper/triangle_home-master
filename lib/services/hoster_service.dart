...
class HosterService {
  ...
  Future<Map<String, dynamic>> getHosterStats(String hosterId) async {
    ...
    return {
      ...
      'pendingBookings': bookings.docs
          .where((doc) => doc.data()['status'] == BookingStatus.inquiryCreated.name)
          .length,
    };
  }
  ...
  Stream<Map<String, dynamic>> getDetailedHosterStatsStream(String hosterId) {
    ...
    final cachedStream = Stream.fromFuture(_isarService.getAdminCache('hoster_stats_$hosterId'))
        .where((c) => c != null)
        .map((c) => json.decode(c!) as Map<String, dynamic>);

    // 2. Real-time Firestore aggregation
    final firestoreStream = Rx.combineLatest4(
      _firestore.collection('properties').where('hoster_id', isEqualTo: hosterId).snapshots(),
      _firestore.collection('bookings').where('hoster_id', isEqualTo: hosterId).snapshots(),
      _firestore.collection('payments').where('hoster_id', isEqualTo: hosterId).snapshots(),
      ...
      (properties, bookings, payments, userDoc) {
        ...
        final activeResidents = bookings.docs.where((doc) {
        ...
        final occupancy = totalCapacity > 0 ? (activeResidents / totalCapacity * 100).round() : 0;
        ...
        final pendingCheckins = bookings.docs.where((doc) {
          final data = doc.data();
        ...
        final paymentsDue = bookings.docs.where((doc) {
          final data = doc.data();
          return data['paymentStatus'] == 'pending' && data['status'] == 'confirmed';
        }).length;

        final bookingsConfirmedToday = bookings.docs.where((doc) {
          final data = doc.data();
        ...
        // Inquiries
        final newInquiries = bookings.docs.where((doc) => doc.data()['status'] == 'pending').length;

        // Calculate profile completion
        int totalFields = 8;
        int filledFields = 0;
        if (hostInfo['name'] != null && hostInfo['name'].toString().isNotEmpty) filledFields++;
        if (hostInfo['email'] != null && hostInfo['email'].toString().isNotEmpty) filledFields++;
        if (hostInfo['phone'] != null && hostInfo['phone'].toString().isNotEmpty) filledFields++;
        ...
        final stats = {
          ...
          'hosterVerified': (userData['permissions'] is Map && userData['permissions']['status'] == 'approved') || (userData['status'] == 'approved'),
          ...
          'chartData': chartData,
          'totalProperties': properties.docs.length,
          'properties': properties.docs.map((doc) => {'id': doc.id, ...doc.data()}).toList(),
          'recentActivity': _generateRecentActivity(bookings.docs, payments.docs),
        };

        // Cache the sanitized result
        _isarService.saveAdminCache('hoster_stats_$hosterId', json.encode(_sanitize(stats)));
  ...
  List<double> _generateRevenueChartData(List<QueryDocumentSnapshot<Map<String, dynamic>>> payments) {
    return [40, 60, 45, 80, 50, 70, 95];
  }

  List<Map<String, dynamic>> _generateRecentActivity(List<QueryDocumentSnapshot<Map<String, dynamic>>> bookings, List<QueryDocumentSnapshot<Map<String, dynamic>>> payments) {
    final List<Map<String, dynamic>> activities = [];
    
    for (var doc in bookings.take(5)) {
      final data = doc.data();
      activities.add({
        'title': '${data['studentName'] ?? "Guest"} booked ${data['roomType'] ?? "Room"} in ${data['propertyName'] ?? "Property"}',
    ...
    for (var doc in payments.take(5)) {
      final data = doc.data();
      activities.add({
        'title': 'Payment received ₹${data['amount']} from ${data['userName'] ?? "User"}',
        'time': _formatTimestamp(data['createdAt']),
        'type': 'payment',
      });
    }
  ...
  Stream<Map<String, dynamic>> getUserProfileStream(String userId) {
    // 1. Initial cached emission
    final cachedStream = Stream.fromFuture(_isarService.getAdminCache('user_profile_$userId'))
        .where((c) => c != null)
        .map((c) => json.decode(c!) as Map<String, dynamic>);

    // 2. Real-time Firestore stream
    final firestoreStream = _firestore.collection('users').doc(userId).snapshots().map((doc) {
      final data = doc.data() ?? {};
      final sanitized = _sanitize(data);
      _isarService.saveAdminCache('user_profile_$userId', json.encode(sanitized));
