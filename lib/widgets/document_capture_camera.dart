...
class _DocumentCaptureCameraState extends State<DocumentCaptureCamera> {
  ...
  void _startStabilityDetection() {
    _userAccelSub = userAccelerometerEventStream().listen((UserAccelerometerEvent event) {
      ...
      bool stableNow = event.x.abs() < threshold && event.y.abs() < threshold && event.z.abs() < threshold;
      
      if (stableNow) {
        if (!_isStable) {
          _isStable = true;
          _stableStartTime = DateTime.now();
        } else if (_stableStartTime != null && _autoCaptureEnabled && !_isCountingDown && !_isCapturing) {
          // If stable for more than 1.5 seconds, start auto-capture
          if (DateTime.now().difference(_stableStartTime!).inMilliseconds > 1500) {
            _startAutoCapture();
          }
        }
      } else {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          IconButton(
            icon: Icon(_autoCaptureEnabled ? Icons.auto_awesome : Icons.auto_awesome_outlined, 
          ...
          if (_isCountingDown)
            Row(
              ...
              children: [
                ...
                Text('Hold steady! Capturing in $_countdown...', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
              ],
            )
            ...
            const Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(Icons.center_focus_strong, color: Colors.white70, size: 20),
                SizedBox(width: 8),
                Text('Align card within the frame', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.w600)),
              ],
            ),
        ],
      ),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          // Flash toggle
          IconButton(
            icon: Icon(_controller?.value.flashMode == FlashMode.torch ? Icons.flash_on : Icons.flash_off, color: Colors.white),
            onPressed: () {
              final newMode = _controller?.value.flashMode == FlashMode.torch ? FlashMode.off : FlashMode.torch;
            ...
            child: Container(
              ...
              child: Center(
                child: Container(
                  ...
                  child: _isCapturing ? const CircularProgressIndicator(color: Colors.blue) : null,
                ),
              ),
            ),
          ),
